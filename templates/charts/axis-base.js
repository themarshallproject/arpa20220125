...
export default class ChartWithAxes extends ChartBase {
  ...
  getScaleExtents() {
    const xMin = isNullOrUndefined(this.config.roundedXMin)
      ? d3.min(this.data, (d) => {
          return this.config.xDataFormat(d[this.config.xKey]);
        })
    ...
    const yMin = isNullOrUndefined(this.config.roundedYMin)
      ? d3.min(this.data, (d) => {
          return this.config.yDataFormat(d[this.config.yKey]);
        })
  ...
  initAxes() {
    ...
    this.yAxis = d3.axisLeft().scale(this.yScale).tickSizeOuter(0);

    this.yGrid = d3.axisLeft().scale(this.yScale).tickFormat('');
  }

  // Add SVG groups where axes will be initialized.
  initAxisElements() {
    this.yGridElement = this.chart.append('g').attr('class', 'y grid');
  ...
  updateAxisElements() {
    ...
    this.xAxisElement
      ...
      .attr(
        'transform',
        `translate(0, ${this.yScale(0) - this.size.chartHeight})`
      );

    this.yAxisElement.call(this.yAxis);

    this.yGridElement.call(this.yGrid.tickSize(-this.size.chartWidth, 0));
  }
