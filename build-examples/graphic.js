...
var _chartBase = _interopRequireDefault(require("charts/chart-base.js"));
...
},{"charts/chart-base.js":34}],2:[function(require,module,exports){
...
function () {
  ...
  function ChartBase(config) {
    ...
    this.$containerEl = $("#".concat(config.containerId));
    this.containerEl = d3.select("#".concat(config.containerId));
  ...
  _createClass(ChartBase, [{
    key: "checkConfigKeys",
    value: function checkConfigKeys(config) {
      this.ensureRequired(config, ['containerId']);
    } // Return error message for each required key missing from config.

  }, {
    key: "ensureRequired",
    value: function ensureRequired(object, requiredKeys) {
      var errors = [];
      requiredKeys.forEach(function (key) {
        if (typeof object[key] === 'undefined') {
          errors.push("Required key ".concat(key, " missing from config options"));
        }
      });

      if (errors.length > 0) {
        console.error("Error calling ".concat(this.constructor.name, ":\n").concat(errors.join('\n')));
      }
    ...
    value: function setConfigDefaults(config) {
    ...
    value: function getSVGHeight() {
      var svgWidth = this.getSVGWidth(); // However, this proportion may need to be expressed through a function
      // rather than a set value.

      var aspectDivisor = this.evaluateOption('aspectRatio');
    ...
    key: "getBaseMeasurements",
    value: function getBaseMeasurements() {
      var svgWidth = this.getSVGWidth();
      var svgHeight = this.getSVGHeight();
      var marginTop = this.evaluateOption('marginTop');
      var marginRight = this.evaluateOption('marginRight');
      var marginBottom = this.evaluateOption('marginBottom');
      var marginLeft = this.evaluateOption('marginLeft');
      return {
    ...
    key: "sizeBaseSVG",
    value: function sizeBaseSVG() {
      this.size = this.getBaseMeasurements(); // The SVG should include margins in its width and height.

      this.svg.attr('width', this.size.chartWidth + this.size.marginLeft + this.size.marginRight).attr('height', this.size.chartHeight + this.size.marginTop + this.size.marginBottom); // Offset the chart group by top and left margins.

      this.chart.attr('transform', "translate(".concat(this.size.marginLeft, ", ").concat(this.size.marginTop, ")"));
    ...
    key: "evaluateOption",
    value: function evaluateOption(optionName, args) {
      var configOption = this.config[optionName];
      var configValue = configOption;
