...
export default class StackedBarChart extends VerticalBarChart {
  initDataElements() {
    this.nacBarRects = this.chart
      ...
      .append('rect')
      .attr('class', (d) => {
        return `bar-rect-nac bar-nac-${utilities.slugify(
          d[this.config.bandKey]
        )}`;
      });
  ...
  initLabels() {
    ...
    this.annotation.append('line').attr('class', 'one-fifty-line');

    this.annotation.append('line').attr('class', 'above-arrow');

    this.annotation.append('text').attr('class', 'more-cases-text');

    this.annotation.append('text').attr('class', 'more-cases-text-1');
  }
  ...
  updateDataElements() {
    ...
    this.barRects
      ...
      .attr('height', (d) => {
        return Math.abs(
          this.yScale(150) - this.yScale(d[this.config.valueKey])
        );
      });
  }
